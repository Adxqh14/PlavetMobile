...
const getEstadoStyles = (vinculador: Vinculador) => {
  if (vinculador.deleted_at) return "bg-red-100 text-red-700 border-red-200";
  if (vinculador.estado === "activo") return "bg-emerald-100 text-emerald-700 border-emerald-200";
  return "bg-gray-100 text-gray-700 border-gray-200";
};

const getEstadoLabel = (vinculador: Vinculador) => {
  if (vinculador.deleted_at) return "Eliminado";
  return vinculador.estado === "activo" ? "Activo" : "Inactivo";
};
      ...
      <DialogContent className="sm:max-w-[550px] max-h-[90dvh] flex flex-col p-0 overflow-hidden border-none shadow-2xl">
        ...
        <div className="relative h-28 bg-linear-to-r from-primary/90 to-primary/70 shrink-0">
          ...
          <div className="absolute top-4 right-4">
            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border shadow-sm backdrop-blur-sm ${getEstadoStyles(vinculador)}`}>
              {getEstadoLabel(vinculador)}
            </span>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-3 rounded-xl bg-muted/30 border border-muted/50 hover:bg-muted/50 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold truncate">{vinculador.email}</p>
                  </div>
                </div>
                <div className="p-3 rounded-xl bg-muted/30 border border-muted/50 hover:bg-muted/50 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold">{vinculador.telefono}</p>
                  </div>
                </div>
              </div>
            </section>
              ...
              <section className="space-y-4">
                ...
                <div className="p-3 rounded-xl bg-muted/30 border border-muted/50">
                  ...
                  <p className="text-sm font-semibold">
                    {new Date(vinculador.fecha_creacion).toLocaleDateString("es-DO")}
                  </p>
                </div>
              </section>
            )}
              ...
              <section className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-destructive/5 border border-destructive/10">
                  <div className="flex items-center gap-2 text-destructive">
                    ...
                    <p className="text-sm font-semibold">
                      Eliminado el:{" "}
                      {new Date(vinculador.deleted_at).toLocaleDateString("es-DO")}
                    </p>
                  </div>
                </div>
              </section>
