      ...
      <DialogContent className="sm:max-w-[550px] max-h-[90dvh] flex flex-col p-0 overflow-hidden border-none shadow-2xl">
        ...
        <div className="relative h-28 bg-linear-to-r from-primary/90 to-primary/70 shrink-0">
          ...
          <div className="absolute top-4 right-4">
            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border shadow-sm backdrop-blur-sm ${getStatusStyles(vinculador.status)}`}>
        ...
        <div className="pt-12 pb-6 px-6 overflow-y-auto flex-1">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-muted-foreground font-medium mt-1 flex items-center gap-2">
              <Fingerprint className="h-3.5 w-3.5" /> {vinculador.cedula || "N/A"} <span className="mx-2">•</span> <Hash className="h-3.5 w-3.5" /> ID: {vinculador.id}
            </p>
          </div>
            ...
            <section className="space-y-4">
              ...
              <div className="p-3 rounded-xl bg-muted/30 border border-muted/50 transition-colors hover:bg-muted/50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold">{vinculador.areaAsignada}</p>
                </div>
              </div>
            </section>

            {/* Información Adicional */}
            {(vinculador.deletedAt || vinculador.status === "deleted") && (
              <section className="space-y-4">
                <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                ...
                <div className="p-4 rounded-xl bg-destructive/5 border border-destructive/10">
                  <div className="flex items-center gap-2 text-destructive">
                    ...
                    <p className="text-sm font-semibold">
                      Inhabilitado el: {vinculador.deletedAt || "Fecha no registrada"}
                    </p>
                  </div>
                </div>
              </section>
