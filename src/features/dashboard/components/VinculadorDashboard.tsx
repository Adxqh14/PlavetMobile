...
export function VinculadorDashboard() {
  ...
  const kpis = [
    { title: "Convenios", value: stats.activeAgreements, desc: "Empresas activas", icon: Building2, color: "text-primary", bg: "bg-primary/10" },
    { title: "Plazas Libres", value: stats.availablePlazas, desc: "Cupos totales", icon: UserPlus, color: "text-emerald-600", bg: "bg-emerald-500/10" },
    { title: "Por Asignar", value: stats.studentsToAssign, desc: "En espera", icon: Users, color: "text-indigo-600", bg: "bg-indigo-500/10" },
    { title: "Alertas", value: 0, desc: "Sin pendientes", icon: AlertCircle, color: "text-rose-600", bg: "bg-rose-500/10" },
  ]
          ...
          <CardContent className="p-4 grid gap-3">
            {[
              ...
              <Button 
                key={i} 
                variant="ghost" 
                className="w-full justify-start font-medium h-20 px-4 hover:bg-muted group rounded-2xl border border-transparent hover:border-border/50 transition-all shadow-xs hover:shadow-sm"
                asChild
              >
                <Link to={action.href}>
          ...
          <div className="p-6 border-t border-border/50 bg-muted/5">
             <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-primary uppercase tracking-widest">Repositorio Legal</p>
                    <p className="text-[11px] text-muted-foreground leading-relaxed">
                      Revisa contratos de convenios y pólizas de seguro institucionales.
                    </p>
                  </div>
                </div>
                <Button variant="outline" size="sm" className="rounded-xl font-bold bg-background">
                  Acceder
                </Button>
             </div>
          </div>
