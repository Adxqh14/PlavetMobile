...
export function SupervisorDashboard() {
  ...
  const kpis = [
    { title: "Estudiantes", value: stats.totalStudents, desc: "En programas activos", icon: Users, color: "text-primary", bg: "bg-primary/10" },
    { title: "Empresas", value: stats.activeCompanies, desc: "Centros de trabajo", icon: Building2, color: "text-emerald-600", bg: "bg-emerald-500/10" },
    { title: "Tasa Éxito", value: stats.successRate, desc: "Promedio global", icon: TrendingUp, color: "text-indigo-600", bg: "bg-indigo-500/10" },
    { title: "Alertas", value: stats.criticalAlerts, desc: "Casos críticos", icon: AlertCircle, color: "text-rose-600", bg: "bg-rose-500/10" },
  ]
  ...
  return (
    <div className="space-y-8 pb-10 animate-in fade-in duration-700">
      ...
      {/* --- KPIs en la parte superior (Full Width) --- */}
            ...
            <CardHeader className="border-b border-border/50 pb-4">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className="text-rose-600 bg-rose-50 border-rose-100 font-bold">
                  {criticalStudents.length} Alertas
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="p-0 flex-1 overflow-auto">
              <div className="divide-y divide-border/50">
                {criticalStudents.map((student) => (
                  <div key={student.id} className="flex items-center justify-between px-6 py-5 hover:bg-muted/30 transition-all group">
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-right hidden sm:block">
                        <p className="text-[10px] font-bold text-rose-600 uppercase tracking-wider leading-none mb-1">{student.motivo}</p>
                        <p className="text-xs font-semibold text-foreground/80">{student.valor}</p>
                      </div>
            ...
            <CardContent className="p-3 grid gap-2 flex-1">
              {[
                ...
                <Button 
                  key={i} 
                  variant="ghost" 
                  className="w-full justify-start font-medium text-sm h-14 px-3 hover:bg-muted group rounded-xl border border-transparent hover:border-border/50 transition-all"
                  asChild
                >
                  <Link to={action.href}>
            ...
            <div className="p-6 mt-auto">
               <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Todos los módulos operativos están funcionando normalmente. El próximo corte de evaluaciones es en 5 días.
                  </p>
               </div>
            </div>
          </Card>
